...
TEST_CASE("Test MemoryCopyTraits", "[MemoryCopyTraits]") {
  ...
  SECTION("PortableCollection<portabletest::TestSoALayout<128, false>, Device>") {
    ...
    for (auto const& device : devices) {
      ...
      auto checkPortableHostCollection = [&queue, &refHostCollection](PortableCollectionType const& col) {
      ...
      std::unique_ptr<ALPAKA_ACCELERATOR_NAMESPACE::ngt::SerialiserBase> serialiserSource{
          ALPAKA_ACCELERATOR_NAMESPACE::ngt::SerialiserFactoryPortable::get()->create(typeName)};
      ...
      edm::Wrapper<PortableCollectionType>* cloneptr =
          dynamic_cast<edm::Wrapper<PortableCollectionType>*>(clonebase.get());
      checkPortableHostCollection(cloneptr->bareProduct());
    }
  }
}
