  ...
  public:
    MPIReceiverPortable(edm::ParameterSet const& config)
      ...
      for (auto const& product : products) {
        ...
        const std::string pdc = "PortableDeviceCollection<";
        if (type.size() > pdc.size()              // type is longer than pdc
            && type.substr(0, pdc.size()) == pdc  // type starts with pdc
            && type.back() == '>')                // type ends with >
        {
          std::string innerType = type.substr(pdc.size(), type.size() - pdc.size() - 1);
        ...
        // Check 2: "type" does not start with "PortableDeviceCollection<".
        // Still, "type" could be the inner type "T" for which a host
        // serialiser (and perhaps portable serialiser as well) exist.
        edm::TypeWithDict twd = edm::TypeWithDict::byName(type);
        std::unique_ptr<ngt::SerialiserBase> hostDeviceSerialiser;
        std::unique_ptr<::ngt::SerialiserBase> hostSerialiser;
        if (twd.typeInfo() != typeid(void)) {
          hostDeviceSerialiser = ngt::SerialiserFactoryDevice::get()->tryToCreate(twd.typeInfo().name());
        ...
        if (hostSerialiser) {
          ...
          if (hostDeviceSerialiser && hostDeviceSerialiser->hasCopyToDevice()) {
            // Register the H to D transform
            entry.token = this->produces(produceInstance)
                              .produces(edm::TypeID{hostDeviceSerialiser->productTypeID()},
                                        edm::TypeID{hostDeviceSerialiser->hostProductTypeID()},
                                        hostDeviceSerialiser->preTransformHtoD(),
                                        hostDeviceSerialiser->transformHtoD());
          } else {
            entry.token = this->producesCollector().template produces<edm::Transition::Event>(typeID, produceInstance);
          }
        ...
        if (twd.typeInfo() == typeid(void) || !wrappedTwd.getClass()) {
          throw cms::Exception("MPIReceiverPortable")
              ...
              << "'. Either register a serialiser via DEFINE_TRIVIAL_SERIALISER_PLUGIN or "
                 "DEFINE_TRIVIAL_SERIALISER_PLUGIN_HOST_DEVICE, or make sure a ROOT dictionary exists for this type.";
        }
        edm::TypeID typeID{twd.typeInfo()};
        entry.token = this->producesCollector().template produces<edm::Transition::Event>(typeID, produceInstance);
