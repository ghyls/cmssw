...
void MPIChannel::wait() {
  ...
  while (state == kBusy) {
    // This is not a busy loop; state_.wait() will block without consuming
    // CPU until other thread calls notify_one() OR this thread is
    // "unblocked spuriously" by the system (hence the while loop).
