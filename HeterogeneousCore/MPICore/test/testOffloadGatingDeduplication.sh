...
# Regression test for how the splitter groups offloaded modules: modules with equal
# reachability signatures must share one offload group, and so one activation
# capture/sender/receiver/filter chain, rather than each getting their own.
#
...
# filter on another Path. So the split must produce 2 gating chains, not 4 (and not 1,
# which is what one group per connected component would give), and -- as a guard against
# merging modules that are not actually equivalent -- every module must still run exactly
# as often as it did unsplit.
...
echo
echo "=== Step 3: check the generated configs collapsed to 2 offload groups ==="
FAIL=0
# the capture and sender live in the local process, the receiver and filter in the remote one
for spec in "activityCaptureBeforeRemote0Group[0-9]*:$LOCAL_PATH" "mpiSenderRemote0Group[0-9]*Activity:$LOCAL_PATH" \
            "mpiReceiverRemote0Group[0-9]*Activity:$REMOTE_PATH" "activityFilterBeforeRemote0Group[0-9]*:$REMOTE_PATH"; do
    kind="${spec%%:*}"
    file="${spec#*:}"
    COUNT=$(grep -c "process\.${kind} = cms\." "$file" || true)
...
echo
if [ "$FAIL" == "0" ]; then
    echo "PASS: gating collapsed to 2 groups and every module's execution count matches baseline."
    exit 0
else
