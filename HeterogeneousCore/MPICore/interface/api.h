...
public:
  ...
  void receiveProduct(Q& queue, int instance, T& product) {
    if constexpr (std::is_fundamental_v<T>) {
      printf("++++++  MPIChannel::receiveProduct: receiving trivial product\n");
      receiveTrivialProduct_(queue, instance, product);
    } else {
      static const TClass* type = TClass::GetClass<T>();
      if (!type) {
        throw std::runtime_error("ROOT dictionary not found for type " + std::string(typeid(T).name()));
      }
      printf("++++++  MPIChannel::receiveProduct: receiving serialized product of type %s\n", type->GetName());
      receiveSerializedProduct_(instance, type, &product);
    }
  }
  ...
  template <typename Q>
  void receiveProduct(Q& queue, int instance, edm::ObjectWithDict& product) {
    // the expected use case is that the product type corresponds to the actual type,
    // so we access the type with typeOf() instead of dynamicType()
    edm::TypeWithDict const& type = product.typeOf();
    if (not type.isFundamental()) {
  ...
  void receiveProduct(int instance, edm::TypeWithDict const& type, edm::WrapperBase& wrapper) {
    if (wrapper.hasTrivialCopyTraits()) {
      printf("++++++  MPIChannel::HERE BE DRAGONS\n");
  ...
  // send surely trivial copy product
  template <typename T>
...
private:
  ...
  // void receiveTrivialProduct_(auto& queue, int instance, edm::ObjectWithDict& product);
...
// receive a wrapped object using its TrivialCopyTraits
  template <typename Q>
void receiveTrivialCopyProduct_(Q& queue, int instance, edm::WrapperBase* wrapper) {
  int tag = EDM_MPI_SendTrivialCopyProduct | instance * EDM_MPI_MessageTagWidth_;
