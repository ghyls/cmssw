...
def split_remote(local_process, args, cpp_names_of_the_products, dependency_graph):
    ...
    for local_dependency, group_indices in producer_to_groups.items():
        ...
        # raw FED data is filed under the synthetic "rawDataCollector" label (issue 45137), so it travels with the Source
        ...
        if local_dependency == "source":
            ...
            # carries the products of the Source and of "rawDataCollector" together
            ...
            if daq_raw_data_products:
                # "rawDataCollector" exists only as a provenance convention, so recreate it as an alias
                alias = create_receiver_alias(receiver_name=receiver_name,
                    products=daq_raw_data_products,
                    module_name="rawDataCollector"
                )
                setattr(remote_process, "rawDataCollector", alias)

            # do the same for every local EDAlias that names this dependency
    ...
    for group_idx, group in enumerate(modules_to_send):
        ...
        # everything this group's members produce, which is what the round trip carries
        group_products = products_of(cpp_names_of_the_products, group)
