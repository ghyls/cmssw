        ...
        case edm::InEvent:
          ...
          for (auto& pattern : eventPatterns_) {
            if (pattern.match(product)) {
              ...
              printf("GetToken: %p\n", (void*)&entry.getToken_);
              printf("PutToken: %p\n", (void*)&entry.putToken_);
  ...
  void GenericClonerPortable::produce(device::Event& event, device::EventSetup const& /*unused*/) {
    for (auto& product : eventProducts_) {
      ...
      if (serialiser) {
        ...
        // TODO: This is bad
        bool isHostMemory = product.objectType_.name().find("PortableHost") != std::string::npos;
        // auto sourcesdev = alpaka::getDev(sources);
        // bool isHostMemory = (sourcesdev == cms::alpakatools::host());
        ...
        for (size_t i = 0; i < sources.size(); ++i) {
          ...
          if (isHostMemory) {
            auto src_view = alpaka::createView(cms::alpakatools::host(), sources[i].data(), sources[i].size_bytes());
            auto trg_view = alpaka::createView(cms::alpakatools::host(), targets[i].data(), targets[i].size_bytes());
