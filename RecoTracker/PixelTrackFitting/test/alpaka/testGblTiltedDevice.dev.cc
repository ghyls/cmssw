// The tilted-OT-barrel measurement model on the device: one real track (eta = 1.087, N = 10) whose OT barrel
// modules at hits 4, 5 and 6 are tilted about 43 deg. Seven variants of the production node chain are solved
// with the device gblFitPca and the host twin:
//   (a) as built        measPrec = inv2(Ruv * ge3 * Ruv^T), the shipping measurement model
//   (b) obliquity-free  measPrec = A^T diag(1/xel, 1/yel) A with A from the true module axes
//   (c) isotropic scat  scatPrec(1,1) := scatPrec(0,0)
//   (d) stiff kinks     scatPrec *= 1e6      (e) MS x0.1  scatPrec *= 10      (f) MS x10  scatPrec *= 0.1
//   (g) all-precise     measPrec := diag(1e8, 1e8)
// Asserted: device equals host twin everywhere; sigma(1/R) is non-decreasing as the kinks loosen,
// (d) <= (e) <= (a) <= (f); (c) <= (a); and sigma(1/R)_(a)/sigma(1/R)_(b) stays inside [kRatioMin, kRatioMax].
