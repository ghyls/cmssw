// Device check of the GBL measurement model: projecting the global 3x3 hit covariance onto the curvilinear
// (U,V) plane and inverting equals the KF-equivalent module-frame measurement through the obliquity Jacobian
// H = A - (1/n.T) c b^T. With (lx, ly, n) the sensor frame, (U, V, T) the curvilinear frame and
//        P = [ A    c  ]   A = [[lx.U, lx.V],[ly.U, ly.V]],  c = (lx.T, ly.T)^T,
//            [ b^T  nT ]   b = (n.U, n.V)^T,                  nT = n.T,
// P is orthogonal, so H = A^-T for any incidence angle and
...
// The comparisons are held at 1e-4 to 1e-3: hits_ge is float, so ge3 is rank two only to ~1e-6, and T is a
// unit vector only on the fast-fit circle. Eigen::SelfAdjointEigenSolver is host-only test algebra.
#include <algorithm>
#include <cmath>
#include <cstdio>
#include <string>
