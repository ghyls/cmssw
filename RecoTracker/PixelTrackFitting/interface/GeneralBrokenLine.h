...
// General Broken Lines (Kleinwort, NIM A 673 (2012) 107), host implementation: an unfactorized 5-parameter
// track fit, equivalent to a Kalman filter plus smoother, as a single (2n+1) bordered-band least-squares
// with full curvilinear helix Jacobians (the analytic propagation of CMSSW's AnalyticalCurvilinearJacobian,
// transcribed into a self-contained function).
// This is the readable reference for the device fit (interface/alpaka/GeneralBrokenLine.h), in dense Eigen
// with dynamic sizes, included only by the unit tests and the replay tool. Its fit-model options default to
// off, while the fit producers drive the device implementation with all of them on; where the two differ,
// the device source is normative.
...
namespace generalBrokenLine {
  ...
  */
  ...
  inline void prepareGblData(const M3xN& hits,
    ...
    // Running deterministic curvilinear state shift, accumulated PCA -> node k, for two known offsets of the
    // reference state transported by the same Jacobians: slot 0 (q/p), the dE/dx spiral, and slot 2 (phi),
