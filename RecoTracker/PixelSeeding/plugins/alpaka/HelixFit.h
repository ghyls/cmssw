...
namespace ALPAKA_ACCELERATOR_NAMESPACE {
  ...
  public:
    ...
    // Dedup union refit and verdict (Phase2OTStubs only). For each contested 0-shared pair {loser i, partner j}
    // captured by Kernel_dedupCovMark, the de-duplicated union of the two hit lists is batch-refitted with GBL;
    // the loser is dropped (drop[i]=1) iff the merged fit converged, its outlier-dropped union hits are <= delta
    // and its chi2/ndof <= max(chi2/ndof_i, chi2/ndof_j); otherwise both are kept, as are unions with more than
    // kRefitMaxN fit-selected hits. contestedPairs holds 2 uint32 per slot {i, j} (unfilled = 0xffffffff);
    // diag, when given, receives the verdict census.
