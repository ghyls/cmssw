...
namespace edm {
  ...
  concept HasValidInitializeAsync =
      ...
      (not HasTrivialCopyProperties<T> &&
       requires(T& object, TQueue& queue) { TrivialCopyTraits<T>::initialize(object, queue); }) ||
      // or does have properties(...) and initialize(object, props) takes two arguments
      (HasTrivialCopyProperties<T> && requires(T& object, TQueue& queue, TrivialCopyProperties<T> props) {
        TrivialCopyTraits<T>::initialize(object, queue, props);
      });
  ...
  template <typename T, typename TQueue>
  concept HasTrivialCopyTraitsAsync =
      // Has memory regions declared
      (HasRegions<T>) &&
      // and has either no initialize(...) or a valid one
      (not requires { &TrivialCopyTraits<T>::initialize; } || HasValidInitializeAsync<T, TQueue>);
