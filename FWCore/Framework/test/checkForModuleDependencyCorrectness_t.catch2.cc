...
TEST_CASE("CheckForModuleDependencyCorrectness", "[Framework]") {
  auto testCase = [](ModuleDependsOnMap const& iModDeps, PathToModules const& iPaths) -> bool {
    PathsAndConsumesOfModulesForTest pAndC(iModDeps, iPaths);
    checkForModuleDependencyCorrectness(pAndC, false);
    return true;
  };

  SECTION("onePathNoCycleTest") {
    {
      ModuleDependsOnMap md = {{"C", {"B"}}, {"B", {"A"}}};
      PathToModules paths = {{"p", {"A", "B", "C"}}};

      REQUIRE(testCase(md, paths));
    }
  ...
  SECTION("onePathHasCycleTest") {
    {
      ModuleDependsOnMap md = {{"C", {"B"}}, {"B", {"A"}}};
      {
        PathToModules paths = {{"p", {"B", "A", "C"}}};

        REQUIRE_THROWS_AS(testCase(md, paths), cms::Exception);
      }
      {
  ...
  SECTION("twoPathsNoCycleTest") {
...

    {
      ...
      PathToModules paths = {
          {"p1", {"E", "F", "zEP1"}}, {"p2", {"B", "C", "D", "zEP2"}}, {"p3", {"zSEP3", "B", "zEP3"}}};
  ...
  SECTION("duplicateModuleOnPathTest") {
    {
      ModuleDependsOnMap md = {{"C", {"B"}}, {"B", {"A"}}};
