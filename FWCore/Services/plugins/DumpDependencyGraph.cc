 ...
 *
 * Attach the Service to a copy of the process and run cmsRun with
 * maxEvents.input = 0:
 */
...
namespace {
  // What EDConsumerBase::checkIfEmpty() substitutes for an InputTag with an
  // empty label
  std::string const kEmptyLabel = "@EmptyLabel@";

  // Get the value of the "@module_edm_type" parameter every module carries in
  // its ParameterSet (e.g. "Source", "EDProducer", etc.)
...
public:
  ...
  static void fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
    ...
    desc.setComment(
        ...
        "\n - consumesNonEvent: the same for the run, lumi and process block transitions,"
        " minus whatever consumes already names"
  ...
  void lookupInitializationComplete(edm::PathsAndConsumesOfModulesBase const& pathsAndConsumes,
    ...
    for (edm::ModuleDescription const* consumer : pathsAndConsumes.allModules()) {
      ...
      std::set<std::string> consumedNonEvent;
      ...
      for (edm::BranchType branchType : {edm::InLumi, edm::InRun, edm::InProcess}) {
        for (edm::ModuleDescription const* produced :
          ...
          consumedNonEvent.insert(produced->moduleLabel());
        }
      }
      ...
      for (edm::ModuleConsumesInfo const& info : pathsAndConsumes.moduleConsumesInfos(consumer->id())) {
        ...
        std::set<std::string>& target = (info.branchType() == edm::InEvent) ? consumed : consumedNonEvent;
      ...
      // A module consuming its own product is not a dependency
      consumed.erase(consumer->moduleLabel());
      consumedNonEvent.erase(consumer->moduleLabel());

      // Don't repeat in consumedNonEvent a label already reported in consumed
      for (std::string const& label : consumed) {
        consumedNonEvent.erase(label);
      }

      // write the three lists, omitting those that are empty
      json& entry = modules_[consumer->moduleLabel()];
      if (not consumed.empty()) {
        entry["consumes"] = consumed;
      }
      if (not consumedNonEvent.empty()) {
        entry["consumesNonEvent"] = consumedNonEvent;
      }
